        ...
        <tr height=30>
            ...
            <td width="60%" align="right" bgcolor="#f7f7f7">
                ...
                <tr>
                    ...
                    <td width="100" align="right"><?=$list[$i][datetime]?>&nbsp;</td>
                </tr>
                </table></td>
            <td rowspan="2" bordercolor="#ececec" background="<?=$board_skin_path?>/img/right_bg.gif"></td>
        </tr>
        <tr>
...
<tr>
    <td>
            ...
            <td bgcolor="#f7f7f7">
                ...
                <tr>
                    ...
                    패스워드 <INPUT type=password maxLength=20 size=15 name="wr_password" itemname="패스워드" required class=ed>
                        <? if ($is_norobot) { ?>
                        <?=$norobot_str?> <INPUT title="왼쪽의 글자중 빨간글자만 순서대로 입력하세요." type="input" name="wr_key" itemname="자동등록방지" required class=ed>
                        <? } ?>
                    <? } ?>

					</td>
                    <? if ($comment_min || $comment_max) { ?><td width=10% align=right><span id=char_count></span>글자</td><?}?>
                </tr>
                </table>
        ...
        <tr>
            <td bordercolor="#ececec" background="<?=$board_skin_path?>/img/left_bg.gif"></td>
            <td bgcolor="#f7f7f7">
                ...
                <? if ($comment_min || $comment_max) { ?>ONKEYUP="check_byte('wr_content', 'char_count');"<?}?> style='width:100%; word-break:break-all;' class=co></TEXTAREA>
                <? if ($comment_min || $comment_max) { ?><script language="JavaScript"> check_byte('wr_content', 'char_count'); </script><?}?>
				
				</td><td width=80 nowrap align=right>		

                <INPUT type="image" src="<?=$board_skin_path?>/img/ok_btn.gif" border=0 accesskey='s'>
